...
def _analysis(mysql):
    ...
    sql = "SELECT V.pattern, count(*) as freq FROM cancer_cui V, meta T where %s and %s group by pattern order by freq desc"  % (curr_condition, filter_builder)
    print sql
    cur.execute(sql)
    pattern_freq_output_simple = []
    pattern_freq_output_simple.append(["pattern", "Frequency"])
    for row in cur.fetchall():
        pattern_freq_output_simple.append([str(row[0]),int(row[1])])
